...
Here we are presenting the top 10 busiest airports, with the size of dots reflecting the nmber of flights through that destination. How "Busy" an airport is, is determined through summing the total flight originating and the total flights incoming that airport. In descending order of flights: 

2) Map the top 10 busiest direct routes. Size of lines should reflect the number of flights through that route.

    ```{r, echo = TRUE, eval = TRUE}

    # Determine the busiest routes in 2002 by filtering by year then grouping by origin and        # destination.
...
Here we are displaying the top 10 busiest direct route, with the size of the lines reflecting the number of flights through that route.
  ...
  # Point 2
    laxFlightDate  %>%
      ...
      ggplot(aes(x = date, y = n)) +
        geom_line() +
        ggtitle("Point 2") +
        geom_label(aes(x = ymd(20041125), y = 1000, label = "2"))
...
# Point 3
    laxFlightDate  %>%
      ...
      ggplot(aes(x = date, y = n)) +
        geom_line() +
        ggtitle("Point 3") +
        geom_label(aes(x = ymd(20040704), y = 1040, label = "3"))
...
The first point shows a sudden drop approximately during the time of 9/11, when the Twin Towers in New York were destroyed by terrorist attack. Airport security measures became more stringent subsequently, and Americans also developed reservations towards traveling via airplane for some time after. It was not until 2004-2005 that flights rebounded to previous levels.
...
Point 5 begins to show an increase in flights as after Christmas/New Years time as they are returning from vacation to their residence/work places.
...
4. Build a predictive model for the arrival delay (`arrdelay`) of flights flying from LAX. Use the same filtering criteria as in the [lecture notes](http://hua-zhou.github.io/teaching/biostatm280-2018winter/slides/12-sparklyr/sparklyr-flights.html) to construct training and validation sets. You are allowed to use a maximum of 5 predictors. The prediction performance of your model on the validation data set will be an important factor for grading this question.
    ...
    Our predictive model predicts the arrival delay of flights from LAX, based on the predictive variables distance departure delay, day of the week, and actual elapsed time. We filter our original data set from a larger subset of years and compared training data and validation data by decile. The bar chart shows the average arrival delay was relatively close. We can compare the model
    A good model should reflect RMSE values similar for the training and test sets of data. A much higher RMSE for the testing set than the training set suggests badly over fit data.
    
  The scatterplot shows that the testing data has a predicted average delay greater than the actual delay time from our model. The Decile Plot shows that the predicted values and actual values based on the validation data match fairly well.
...
I have interest in visualizing the top flight locations that fly toward San Francisco.
